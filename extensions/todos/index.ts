...
/**
 * Local keybinding helpers to avoid depending on getEditorKeybindings()
 * which has ESM/CJS interop issues in some setups.
 */
const SELECT_KEYS = {
	selectUp: [Key.up],
	selectDown: [Key.down],
...
function matchesAction(keyData: string, action: keyof typeof SELECT_KEYS): boolean {
	for (const key of SELECT_KEYS[action]) {
		if (matchesKey(keyData, key)) return true;
	}
	return false;
}
...
class TodoSelectorComponent extends Container implements Focusable {
	...
	handleInput(keyData: string): void {
		if (matchesAction(keyData, "selectUp")) {
		...
		if (matchesAction(keyData, "selectDown")) {
			if (this.filteredTodos.length === 0) return;
			this.selectedIndex = this.selectedIndex === this.filteredTodos.length - 1 ? 0 : this.selectedIndex + 1;
			this.updateList();
			return;
		}
		if (matchesAction(keyData, "selectConfirm")) {
			const selected = this.filteredTodos[this.selectedIndex];
			if (selected) this.onSelectCallback(selected);
			return;
		}
		if (matchesAction(keyData, "selectCancel")) {
...
class TodoDetailOverlayComponent {
	...
	handleInput(keyData: string): void {
		...
		if (matchesAction(keyData, "selectPageUp") || matchesKey(keyData, Key.left)) {
			this.scrollBy(-this.viewHeight || -1);
			return;
		}
		if (matchesAction(keyData, "selectPageDown") || matchesKey(keyData, Key.right)) {
			this.scrollBy(this.viewHeight || 1);
			return;
		}
	}
